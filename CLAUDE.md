...
**`lib/graph.dart` is the second thing the directive scan is good for.** `sort.directiveUris` runs the sorter's own `_SourceScanner`, so a commented-out import never becomes an edge, and it reads `part` as well — leaving it out would make every generated `.g.dart` look unreferenced. `ImportGraph` is pure and takes project-relative POSIX paths; `resolveUri` maps `package:<self>/x` to `lib/x` and walks relative paths, returning null for anything outside the project. Cycles come from iterative Tarjan (recursion would risk the stack on a deep project); `unreferenced` is in-degree zero rather than reachability, which misses a dead pair that imports itself and is deliberate — a report people stop trusting is worse than one that misses something. `--report` is read-only and exits 1 only under `--exit-if-changed`.
