...
**CI gates beyond the three commands above.** `tool/check_changelog.dart` fails when `CHANGELOG.md` does not open on the `pubspec.yaml` version or its sections are not newest-first — Release Please has inserted a section in the wrong place twice. `tool/check_coverage.dart` fails under 85% line coverage of `lib/` (93.1% today); `bin/` runs as a subprocess in the CLI tests and is not instrumented, so it is deliberately out of scope. `dart pub publish --dry-run` runs on PRs. Publishing itself happens from a `vX.Y.Z` tag over OIDC (`.github/workflows/publish.yml`), which needs automated publishing enabled on the package's pub.dev admin page.
